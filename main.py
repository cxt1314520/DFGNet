...
from models.model import DFGNet
import yaml
import setproctitle
...
def main(**kwargs):
    ...
    model_name      = config['start_up']['model_name']
    setproctitle.setproctitle("{0}.{1}@SXJP".format(model_name, dataset_name))
    ...
    # init the model
    model_args['manifold'] = Hyperboloid
    model_args['c'] = None
    model   = DFGNet(**model_args).to(device)
...
#                print('33333',trainx.shape,trainy.shape)
            ...
            torch.save(results, 'all_epochs_results.pth')
